...
def MMD_reg_f_div_flow(
        ...
        div_der = tsallis_der,
        target_name = 'circles',
        verbose = False, # decide whether to print warnings if (pseudo / relative) dual gaps are too high
        compute_W2 = False,
        save_opts = False,
        compute_KALE = False,
        st = 42
    ...
    if target_name == 'cross':
        ...
        samples1 = rotate_points(samples, np.pi/2)
        samples2 = rotate_points(samples, np.pi)
        samples3 = rotate_points(samples, np.pi*3/2)
    ...
    # state recession constant and embedding constant
    if divergence == 'tv':
        f_infty_prime = 1
    if kernel == 'IMQ':
        B = 1/sigma**(1/4)    
    ...
    for n in tqdm(range(iterations)):
        ...
        '''
        threshold = lambd - 2*MMD[n]*B / f_infty_prime
        if threshold > 0:
            print("lambda is admissable")
        else:
            print("lambda is not admissable")
        '''
...
def this_main(
    ...
    suffix = f'{lambd},{step_size},{N},{kernel},{sigma},{max_time},{target_name}'
    torch.save(func_values, f'{folder}/Reg_{diverg}_Div_value_timeline,{suffix}.pt')
    torch.save(MMD_values, f'{folder}/Reg_{diverg}_Div_MMD_timeline,{suffix}.pt')
    torch.save(W2_values, f'{folder}/Reg_{diverg}_Div_W2_timeline,{suffix}.pt')
    fig, ax = plt.subplots()
    for k in range(L):
    ...
    ax.spines['top'].set_visible(False)
    ax.spines['right'].set_visible(False)
    plt.savefig(f'{folder}/Reg_{diverg}_func_timeline,{suffix}.png', dpi=300, bbox_inches='tight')
    plt.close()
    ...
    ax.spines['top'].set_visible(False)
    ax.spines['right'].set_visible(False)
    plt.savefig(f'{folder}/Reg_{diverg}_MMD_timeline,{suffix}.png', dpi=300, bbox_inches='tight')
    plt.close()
    ...
    if compute_KALE:
      ...
      ax.spines['top'].set_visible(False)
      ax.spines['right'].set_visible(False)
      plt.savefig(f'{folder}/Reg_{diverg}_KALE_timeline,{suffix}.png', dpi=300, bbox_inches='tight')
      plt.close()
    ...
    if compute_W2:
        ...
        ax.spines['top'].set_visible(False)
        ax.spines['right'].set_visible(False)
        plt.savefig(f'{folder}/Reg_{diverg}_W2_timeline,{suffix}.png', dpi=300, bbox_inches='tight')
        plt.close()
...
# pr = cProfile.Profile()
# pr.enable()
MMD_reg_f_div_flow(sigma = .5,
    step_size = 1e-3,
    max_time = 1e1,
    lambd = 1e-2,
    N = 200*3,
    kern = inv_log,
    kern_der = inv_log_der,
    target_name = 'cross',
    alpha = 3,
    div = tsallis,
    div_der = tsallis_der,

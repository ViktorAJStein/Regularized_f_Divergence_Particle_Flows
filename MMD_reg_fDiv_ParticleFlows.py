...
def MMD_reg_f_div_flow():
    ...
    gif = False # produces gif showing the evolution of the particles
    timeline = False # plots timeline of functional value along the iterative scheme
    ...
    for n in range(iterations):
        ...
        if kern == inv_multiquadric:
            K = (distances.pow(2) + sigma).pow(-1/2).numpy()
        
        if kern == gauss:
            K = torch.exp(-distances.pow(2) / (2 * sigma**2)).numpy()
        k_end = time.time()
        ...
        if mode == 'dual':
            ...
            q_torch = torch.tensor(q_np, dtype=torch.float64) # torch version of solution vector
            end_ne = time.time()
            print(f"Rest took {end_ne - start_ne} seconds")
            # if timeline:
            ...
            g_end = time.time()
            print(f"Gradient update took {g_end - g_start} seconds")

...
# def riesz(x, y = np.ones(10), s = 3/4):
#     return LA.norm(x)**(2*s) + LA.norm(y)**(2*s) - LA.norm(x - y) ** (2*s)

# def riesz_der(x, y = np.ones(10), s = 3/4):
...
def DALE_flow(prior, # initial configuration of particles
              ...
              gif = False, # decide whether to make a gif from simulation
              timeline = False, # decide whether to plot timeline of DALE value along iteration
              ...
              div = tsallis,
              div_der = tsallis_der,
              target_name = 'two circles' # choose the target measure nu
              ):
    ...
    if plot or gif or timeline:
        folder_name = f"{divergence},alpha={alpha},lambd={lambd},tau={step_size},{kernel},{sigma},{mode},{max_time}"
    ...
    for n in range(iterations):
        ...
        if plot and not n % int(1/(10*step_size)):
            ...
            plt.savefig(folder_name + f'/{divergence}{alpha}_DALE_flow,lambd={lambd},tau={step_size}-{time_stamp}.png', dpi=300)
            plt.show()
            plt.close()
    end_time = time.time()
    ...
    if timeline:
        ...
        title2 = fr'$\alpha$ = {alpha}, $\lambda$ = {lambd}, $\tau$ = {step_size}, $N$ = {N},' + kernel + fr' kernel, $\sigma^2$ = {sigma}'
    ...
    return DALE_value, elapsed_time
...
# title1 = f'{divergence}-DALE objective value. Problem solved via the ' + mode + ' problem, \n'
# title2 = fr'$\lambda$ = {lambd}, $\tau$ = {step_size}, $N$ = {N}, ' + kernel + fr' kernel, $\sigma^2$ = {sigma}'
# plt.title(title1 + title2)
# plt.legend(frameon = False)
# plt.yscale('log')
# plt.xlabel(r'time $t$')
